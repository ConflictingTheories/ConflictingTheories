...
def = {
    ...
    drawOffset: Vector.create([-0.25, 1, 0.125]),
    hotspotOffset: Vector.create([0.5, 0.5, 0]),
    ...
    tick: function(engine, time) {
        ...
        if (this.bindCamera)
            Vector.set(this.pos, engine.cameraPosition);
    },
    ...
    checkInput: function() {
        ...
        var from = Vector.create(this.pos);
        var dp = Direction.toOffset(facing);
        var to = Vector.create([Math.round(from[0] + dp[0]), Math.round(from[1] + dp[1]), 0]);
